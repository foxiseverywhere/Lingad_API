...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if ($req[0] == 'get_Users_ID' && isset($req[1])) {
            $userId = $req[1];
    ...
    case 'POST':
        ...
        if ($req[0] == 'ins_User') {
            echo json_encode($try->insert_User($data_input));
            return;
        }
        break;

    case 'PUT':
        if ($req[0] == 'upd_User' && isset($req[1])) {
            $userId = $req[1];
